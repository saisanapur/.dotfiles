...
- **No upper-layer use-case comments on lower-layer APIs.** In any layered architecture, do not document callers, product flows, or "pass X when …" scenarios on repositories, clients, shared libs, or other dependencies — that couples the lower layer to today's consumers. Put call-specific rationale at the call site (or nowhere); lower-layer comments only for mechanics that layer owns which the signature cannot express. (Cursor rule: `cursor/rules/no-lower-layer-use-case-comments.mdc`.)
